...
class LabEquipment(db.Model):
    ...
    size_value = db.Column(db.Float, nullable=True)  # e.g., 500
    size_unit = db.Column(db.String(5), nullable=True)  # e.g., mL, L, mm
...
class Reservation(db.Model):
    ...
    admin_notes = db.Column(db.String(200))
...
def create_equipment(current_user):
    ...
    required_fields = ['name', 'labID', 'total_quantity', 'size_value', 'size_unit']
...
def init_db():
    with app.app_context():
        ...
        labs = [
            Laboratory(lab_name='Biology Lab'),
            Laboratory(lab_name='Chemistry Lab'),
            Laboratory(lab_name='Botany Lab'),
            Laboratory(lab_name='Biochemistry Lab')
        ]
