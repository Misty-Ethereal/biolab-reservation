    ...
    reservations.forEach(reservation => {
      if (reservation.status === 'approved' || reservation.status === 'rejected' || reservation.status === 'returned') {
        history.push({
          ...
          equipmentSize: reservation.equipment?.size_value && reservation.equipment?.size_unit
            ? `${reservation.equipment.size_value} ${reservation.equipment.size_unit}`
            : '-',
  ...
  const handleEditEquipment = async (equipmentId: string) => {
    ...
    if (equip) {
      ...
      setNewEquipment({
        ...
        size_value: equip.size_value || 0,
        size_unit: equip.size_unit || 'mL',
      });
                ...
                {otherReservations
                  ...
                  .map((reservation) => (
                    <TableRow key={reservation.reservationID}>
                      <TableCell>{reservation.user_name}</TableCell>
                      <TableCell>{reservation.equipment_name}</TableCell>
                      <TableCell>{reservation.equipment?.size_value && reservation.equipment?.size_unit
                          ? `${reservation.equipment.size_value} ${reservation.equipment.size_unit}`
                          : '-'}
                      </TableCell>
                      <TableCell>{reservation.quantity}</TableCell>
              ...
              <TableBody>
                {filteredEquipment.map((equipment) => (
                  <TableRow key={equipment.equipmentID}>
                    ...
                    <TableCell>
                      {equipment.size_value && equipment.size_unit 
                        ? `${equipment.size_value} ${equipment.size_unit}` 
                        : '-'}
                    </TableCell>
                    <TableCell>
              ...
              <TableBody>
                {filteredHistory.map((action) => (
                  <TableRow key={`${action.reservationID}-${action.type}`}>
                    ...
                    <TableCell>{action.equipmentSize || '-'}</TableCell>
      ...
      <Dialog open={openEquipmentDialog} onClose={() => setOpenEquipmentDialog(false)}>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            type="number"
            label="Size Value"
            value={newEquipment.size_value}
            onChange={(e) => setNewEquipment({ ...newEquipment, size_value: parseFloat(e.target.value) })}
            sx={{ mb: 2 }}
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              value={newEquipment.size_unit}
              label="Size Unit"
              onChange={(e) => setNewEquipment({ ...newEquipment, size_unit: e.target.value })}
            >
        ...
        <DialogActions>
          ...
          <Button 
            ...
            variant="contained" 
            color="primary"
            disabled={!newEquipment.name || !newEquipment.labID || newEquipment.size_value === undefined}
          >
